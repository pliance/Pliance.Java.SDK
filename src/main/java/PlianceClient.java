...
import Exceptions.PlianceApiException;
	...
	public RegisterPersonResponse RegisterPerson(RegisterPersonCommand command) throws Exception {
		...
		return Execute("api/PersonCommand", (client) -> {
			try {
				...
				OutputStreamWriter stream = new OutputStreamWriter(client.getOutputStream());
				...
				String data = Convert(client.getInputStream());

				return gson.fromJson(data, RegisterPersonResponse.class);
			} catch (Exception e) {
